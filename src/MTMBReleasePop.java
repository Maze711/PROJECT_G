...
public class MTMBReleasePop {
	...
	private void fetchDataFromDatabase(String input) {
		String query = "SELECT CtrlNo FROM 2024mtmbrecord WHERE CtrlNo LIKE ?";

		try (Connection conn = this.conn.getConnection(); PreparedStatement statement = conn.prepareStatement(query)) {
	...
	private void searchButtonActionPerformed(ActionEvent e) {
		...
		String status = fetchStatusFromDatabase(input);
		textField_1.setText(status);

		comboBox.setVisible(false);
	}

	private String fetchStatusFromDatabase(String ctrlNo) {
		String status = "";
		String query = "SELECT Status FROM 2024mtmbrecord WHERE CtrlNo = ?";
	...
	private void updateStatusInDatabase(String ctrlNo, String status) {
	    String query = "UPDATE 2024mtmbrecord SET Status = ? WHERE CtrlNo = ?";
